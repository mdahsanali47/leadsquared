...
from fastapi import (APIRouter, Cookie, Depends, FastAPI, File, Form, HTTPException,
                     Request, Response, UploadFile, status) # <-- Import APIRouter
...
async def process_reports_endpoint(start_date: str = Form(...), end_date: str = Form(...), planned_visit_file: UploadFile = File(...), unplanned_visit_file: UploadFile = File(...), counters_file: UploadFile = File(...), users_file: UploadFile = File(...), access_token: Optional[str] = Depends(get_token_from_cookie)):
